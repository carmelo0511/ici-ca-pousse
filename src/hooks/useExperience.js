...
function calculateWorkoutXP(workout, previousWorkouts = []) {
  ...
  if (previousWorkouts.length > 0) {
    ...
    const daysDiff = Math.floor(
      (currentWorkoutDate - lastWorkoutDate) / (1000 * 60 * 60 * 24)
    );
...
export const useExperience = (user) => {
  ...
  const addWorkoutXP = useCallback(
    async (workout, previousWorkouts = []) => {
      if (!user?.uid) return;
  ...
  const addBadgeUnlockXP = useCallback(
    async (badgeName) => {
      return await addXP(
        XP_REWARDS.BADGE_UNLOCK,
  ...
  const addChallengeSendXP = useCallback(
    async (challengeName) => {
      return await addXP(
        XP_REWARDS.CHALLENGE_SEND,
        `Défi envoyé: ${challengeName}`
      );
    },
    [addXP]
  );
  ...
  const addChallengeWinXP = useCallback(
    async (challengeName, customXP = null) => {
  ...
  const addStreakMilestoneXP = useCallback(
    async (streakCount) => {
      return await addXP(
        XP_REWARDS.STREAK_MILESTONE,
        `Streak de ${streakCount} jours !`
      );
    },
    [addXP]
  );
  ...
  const addWeeklyGoalXP = useCallback(
    async (goalName) => {
      return await addXP(
        XP_REWARDS.WEEKLY_GOAL,
        `Objectif hebdomadaire: ${goalName}`
      );
    },
    [addXP]
  );
  ...
  const addMonthlyGoalXP = useCallback(
    async (goalName) => {
      return await addXP(
        XP_REWARDS.MONTHLY_GOAL,
        `Objectif mensuel: ${goalName}`
      );
    },
    [addXP]
  );
  ...
  const addPersonalRecordXP = useCallback(
    async (exerciseName) => {
      return await addXP(
        XP_REWARDS.PERSONAL_RECORD,
        `Record personnel: ${exerciseName}`
      );
    },
    [addXP]
  );
