...
export const useChallenges = (user, addChallengeSendXP, addChallengeWinXP) => {
  ...
  const challengeTypes = [
    ...
    {
      id: 'morning_workouts',
      label: 'Séances matinales',
      icon: '🌅',
      category: 'regularity',
    },
    {
      id: 'weekend_workouts',
      label: 'Séances weekend',
      icon: '🎯',
      category: 'regularity',
    },
  ...
  const calculateChallengeLevel = useCallback(
    (score, type, target) => {
  ...
  const calculateChallengeRewards = useCallback(
    (score, type, target, duration) => {
      ...
      const finalXP = Math.round(
        baseReward.xp * baseReward.multiplier * durationMultiplier
      );
  ...
  const createChallenge = useCallback(
    async (challengeData) => {
      ...
      const newChallenge = {
        ...
        receiverName:
          challengeData.friend.displayName || challengeData.friend.email,
      ...
      try {
        ...
        const start =
          typeof challenge.startDate === 'string'
            ...
            : challenge.startDate;
        const end =
          typeof challenge.endDate === 'string'
        ...
        switch (challenge.type) {
          ...
          case 'progression': {
            ...
            return exerciseCount > 0
              ? Math.round(totalProgression / exerciseCount)
              : 0;
          }
          ...
          case 'weekly_consistency': {
            ...
            return weeklyAverages.length > 0
              ? Math.round(
                  weeklyAverages.reduce((a, b) => a + b, 0) /
          ...
          case 'strength_sessions': {
            ...
            return filteredWorkouts.filter((workout) =>
              workout.exercises?.some(
                (exercise) =>
                  exercise.type !== 'cardio' && exercise.type !== 'custom'
              )
            ).length;
          }
          ...
          case 'intensity': {
            ...
            const intensities = filteredWorkouts.map((workout) => {
              ...
              return exerciseCount > 0
                ? (totalIntensity / exerciseCount) * 100
                : 0;
            });
    ...
    (challenge) => {
      ...
      if (now > endDate) {
        ...
        if (currentScore !== myScore) {
          ...
          updateChallenge(challenge.id, { [scoreField]: myScore }).catch(
            (error) => {
        ...
        if (myFinalScore > friendFinalScore) {
          const rewards = calculateChallengeRewards(
            myScore,
            challenge.type,
            challenge.target,
            challenge.duration
          ...
          if (challenge.status !== 'completed' && addChallengeWinXP) {
            updateChallenge(challenge.id, {
              ...
              addChallengeWinXP(challengeName, xpToAdd)
                ...
                .catch((error) => {
                  console.error(
                    "Erreur lors de l'ajout d'XP pour victoire:",
                    error
                  );
                });
            });
        ...
        if (friendFinalScore > myFinalScore)
          return { status: 'defeat', text: 'Défaite 😔' };
