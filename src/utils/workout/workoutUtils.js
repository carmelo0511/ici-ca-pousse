      ...
      const exercises = (w.exercises || [])
        .map((ex) => {
          const weights = (ex.sets || [])
            .map((set) =>
              set.weight && parseFloat(set.weight) > 0 ? set.weight : '-'
            )
            .join('/');
      ...
      const exercises = (w.exercises || [])
        .map((ex) => {
          ...
          const avgReps =
            sets.length > 0
              ? Math.round(
                  sets.reduce((sum, s) => sum + (parseFloat(s.reps) || 0), 0) /
                    sets.length
                )
              : 0;
          // Moyenne du poids
          ...
          if (avgWeight > 0) poidsStr = avgWeight + ' kg';
            ...
            ex.name.toLowerCase().includes('traction') ||
            ex.name.toLowerCase().includes('pompe')
          )
            poidsStr = 'au poids du corps';
