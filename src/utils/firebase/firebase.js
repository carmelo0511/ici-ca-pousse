...
export async function createChallengeInFirebase(challengeData) {
  try {
    ...
    console.error(
      '🔥 Erreur lors de la création du défi dans Firebase:',
      error
    );
    throw error;
  }
...
export async function getChallengesFromFirebase(userId) {
  try {
    ...
    const sentQuery = query(challengesRef, where('senderId', '==', userId));
...
export async function uploadProfilePicture(userId, file) {
  try {
    ...
    if (error.code === 'storage/unauthorized') {
      throw new Error(
        "Erreur d'autorisation. Vérifiez que vous êtes connecté."
      );
