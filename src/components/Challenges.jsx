...
const Challenges = ({ user }) => {
  ...
  const handleCreateChallenge = async () => {
    ...
    try {
      ...
      try {
        ...
        setToast({
          message: 'Défi créé et notification envoyée !',
          type: 'success',
        });
      } catch (error) {
        console.error("Erreur lors de l'envoi de la notification:", error);
        setToast({
          message: 'Défi créé mais erreur de notification',
          type: 'warning',
        });
      }
  ...
  // Calcul des statistiques (désactivé - composant ChallengeStats supprimé)
  ...
  //   const winRate =
  //     totalChallenges > 0 ? Math.round((victories / totalChallenges) * 100) : 0;
  ...
  const renderChallengeRewards = (challenge) => {
    ...
    const myScore = getChallengeScore(challenge);
    const achievedLevel = calculateChallengeLevel(
      myScore,
      challenge.type,
      challenge.target
    );
    ...
    <div className="p-4 w-full min-h-screen h-auto">
      ...
      <div className="pt-6 mb-6 pl-6">
        ...
        <p className="text-gray-600 mt-1">
          Affrontez vos amis dans des défis sportifs
        </p>
      </div>
      ...
      {/* Statistiques des défis - Temporairement désactivées */}
      ...
      {notifications.filter((n) => n.type === 'challenge_invite' && !n.read)
        .length > 0 && (
        <Card className="mb-6 border-l-4 border-l-yellow-500 bg-yellow-50">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() =>
                notifications
                  .filter((n) => n.type === 'challenge_invite' && !n.read)
      ...
      {activeTab === 'all' &&
        (() => {
          const activeChallenges = getActiveChallenges().filter(
            (challenge) => !hiddenChallenges.has(challenge.id)
          );
              ...
              <Card>
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600 mb-4">
                    Crée ton premier défi ou attends d'en recevoir un !
                  </p>
                </div>
              </Card>
            );
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        {isPending && !isSentByMe ? (
                          ...
                          <>
                            <div
                              className={`font-bold ${getStatusColor(status.status)}`}
                            >
                              {status.text}
                            </div>
                            <div className="text-sm text-gray-600">
                              {formatScore(myScore, challenge.type)} vs{' '}
                              {formatScore(
                                getFriendScore(challenge),
                                challenge.type
                              )}
                            </div>
                            {isStreakChallenge && myScore === 0 && (
                              <div className="text-xs text-red-500 mt-1">
                                Aucune série consécutive pendant la période du
                                défi.
                              </div>
                            )}
                            {isPending && isSentByMe && (
        ...
        (sentChallenges.length === 0 ? (
          <Card>
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                Crée ton premier défi avec un ami !
              </p>
            </div>
          </Card>
        ) : (
                ...
                <Card key={challenge.id}>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-gray-600">
                        {formatScore(myScore, challenge.type)} vs{' '}
                        {formatScore(getFriendScore(challenge), challenge.type)}
                      </div>
        ...
        (receivedChallenges.length === 0 ? (
          <Card>
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">
                Tes amis peuvent t'envoyer des défis !
              </p>
            </div>
          </Card>
        ) : (
      ...
      {activeTab === 'completed' &&
        (() => {
          ...
          const hiddenCompletedChallenges = completedChallenges.filter(
            (challenge) => hiddenChallenges.has(challenge.id)
          );
            ...
            return (
              <Card>
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600 mb-4">
                    Termine tes premiers défis pour les voir ici !
                  </p>
                </div>
              </Card>
            );
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {hiddenCompletedChallenges.length} défi(s) terminé(s)
                          masqué(s)
                        </p>
                      </div>
                    </div>
                    <button
                      onClick={() =>
                        hiddenCompletedChallenges.forEach((challenge) =>
                          handleShowChallenge(challenge.id)
                        )
                      }
                      className="text-sm text-blue-600 hover:text-blue-800"
                    >
        ...
        <div className="p-6">
          ...
          <div className="mb-4">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-full transition-all ${
                      challengeTypes.some(
                        (t) => t.category === category && challengeType === t.id
                      )
                        ? 'bg-blue-500 text-white'
                        : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                    }`}
                    onClick={() => {
                      const firstTypeInCategory = challengeTypes.find(
            ...
            <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto">
              {challengeTypes
                .filter((type) => {
                  const selectedCategory = challengeTypes.find(
                    (t) => t.id === challengeType
                  )?.category;
