...
const Header = memo(({ workoutCount, className = '', user, workouts = [], challenges = [], addBadgeUnlockXP, onUserUpdate, refreshUserProfile }) => {
  ...
  return (
    <header className={`backdrop-blur-md bg-gradient-to-r from-indigo-500/80 to-purple-600/80 shadow-xl rounded-b-2xl border-b border-indigo-200 dark:border-gray-800 ${className}`}>
      {/* Ultra-compact sur mobile : tout sur une ligne */}
      <div className="flex flex-row items-center justify-between w-full px-1.5 py-1 sm:px-8 sm:py-4 gap-1 sm:gap-3 min-h-0">
        ...
        {/* Titre centré au milieu du header */}
        <div className="flex-1 text-center min-w-0">
          <h1 className="text-base sm:text-lg md:text-xl font-extrabold text-black dark:text-white drop-shadow tracking-tight truncate">
            Ici Ça Pousse
          </h1>
        </div>
        {/* Profil et actions */}
