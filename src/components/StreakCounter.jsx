...
const StreakCounter = ({ streak, className = '' }) => {
  ...
  return (
    <div
      className={`flex items-center space-x-1 bg-white/90 dark:bg-gray-800/90 rounded-lg px-2 py-1 border border-white/50 dark:border-gray-700/50 shadow-sm backdrop-blur-sm ${className}`}
      style={{ minWidth: 0 }}
    >
      <span
        className={`text-sm ${getStreakColor(streak)} mr-1`}
        style={{ minWidth: 16 }}
      >
      ...
      <div className="flex flex-col items-center">
        ...
        {getStreakTitle(streak) && (
          <span
            className={`text-[8px] font-bold ${getStreakColor(streak)} uppercase tracking-wide leading-tight text-center`}
          >
            {getStreakTitle(streak)}
          </span>
        )}
