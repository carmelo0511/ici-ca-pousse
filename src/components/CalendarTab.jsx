        ...
        <div className="grid grid-cols-7 gap-2">
          {days.map((day, index) => {
            ...
            return (
              <div
                key={day}
                className={`h-12 flex items-center justify-center text-sm rounded-xl cursor-pointer font-medium transition-all duration-200 relative ${
                  isToday
              ...
              {selectedWorkout.exercises.map((exercise, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-sm font-bold ${exercise.type === 'cardio' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}
                    >
                  ...
                  <div className="space-y-3">
                    <div
                      className={`grid gap-3 text-sm font-bold text-gray-600 pb-2 border-b border-gray-300 ${exercise.type === 'cardio' ? 'grid-cols-3' : 'grid-cols-3'}`}
                    >
                      <span>Série</span>
                      {exercise.type === 'cardio' ? (
                        <>
                    ...
                    {exercise.sets.map((set, setIndex) => (
                      <div
                        key={setIndex}
                        className="grid grid-cols-3 gap-3 text-sm font-medium"
                      >
                        <span className="bg-white rounded-lg px-3 py-2 text-center border border-gray-200">
                          {setIndex + 1}
                        </span>
                        {exercise.type === 'cardio' ? (
                          <>
