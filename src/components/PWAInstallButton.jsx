...
const PWAInstallButton = () => {
  ...
  // iOS: afficher une info bulle si non installée
  if (isIOS() && !isInStandaloneMode() && !isInstalled) {
    return (
      <div className="fixed bottom-4 right-4 z-50 bg-white/90 border border-indigo-200 text-indigo-700 px-4 py-2 rounded-xl shadow-lg flex items-center space-x-2 text-xs md:text-sm pulse-soft">
        <Info className="h-4 w-4 md:h-5 md:w-5" />
        <span>
          {i18n.language === 'fr'
            ? "Pour installer l'app, ouvrez le menu de Safari puis 'Sur l'écran d'accueil'"
