...
export function sendCongratsAfterWorkout({
  user,
  workout,
  workouts,
  setMessages,
  ...
  if (workout && workout.exercises) {
    for (const ex of workout.exercises) {
      const maxWeight = Math.max(
        ...(ex.sets?.map((s) => Number(s.weight) || 0) || [0])
      );
      // Cherche si c’est le max historique
      const allWeights = workouts.flatMap(
        (w) =>
          w.exercises
  ...
  setMessages((prev) => [
    ...
    {
      role: 'assistant',
      content: `${prenom ? prenom + ', ' : ''}bravo pour ta régularité ! Tu viens d’enchaîner ${nbThisWeek} séance${nbThisWeek > 1 ? 's' : ''} cette semaine 👏${recordMsg ? '\n' + recordMsg : ''}`,
    },
  ]);
}
