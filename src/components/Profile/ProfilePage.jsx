...
const ProfilePage = ({ user, workouts = [], challenges = [], onUserUpdate, addBadgeUnlockXP, refreshUserProfile }) => {
  ...
  const handleSaveGoal = React.useCallback(async (selectedGoal) => {
    setLoading(true);
    setError('');
    setSuccessMessage('');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <form onSubmit={handleSavePhysical} className="mb-4 flex flex-col gap-4 items-center">
          <div className="flex gap-4 flex-wrap justify-center">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Surnom (max 10)</label>
              <input
                type="text"
                maxLength={10}
                value={nickname}
        ...
        {successMessage && <div className="text-green-600 text-sm text-center bg-green-50 p-3 rounded-lg">{successMessage}</div>}
      </div>

      {/* Section Objectifs */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 className="font-semibold text-lg mb-4 text-center">Définir mon objectif</h3>
      ...
      {/* Section badges */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 className="font-semibold text-lg mb-4 text-center">Choisir un badge comme avatar</h3>
        {badges.length === 0 ? (
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4 max-h-96 overflow-y-auto">
            {badges.map((badge) => (
              ...
              >
                ...
                <span className="text-xs text-gray-700 text-center mt-2">{BADGE_CONFIG[badge].name}</span>
                {localSelectedBadge === badge && (
                  <span className="text-indigo-500 text-xs mt-1 font-medium">✓ Sélectionné</span>
                )}
              </button>
            ))}
          </div>
