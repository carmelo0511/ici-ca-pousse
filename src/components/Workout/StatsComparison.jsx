...
function StatsComparison({ user }) {
  ...
  const fetchFriendStats = async (friend) => {
    ...
    const friendWorkouts = snap.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
    ...
    const userWorkouts = userSnap.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
              ...
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-4 rounded-xl">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-2xl font-bold text-blue-900">
                    {comparisonData.user.totalWorkouts}
                  </span>
                  <span className="text-lg text-blue-700">
                    vs {comparisonData.friend.totalWorkouts}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-xl">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-2xl font-bold text-green-900">
                    {comparisonData.user.avgDuration}min
                  </span>
                  <span className="text-lg text-green-700">
                    vs {comparisonData.friend.avgDuration}min
                  </span>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-4 rounded-xl">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-2xl font-bold text-purple-900">
                    {comparisonData.user.avgExercises}
                  </span>
                  <span className="text-lg text-purple-700">
                    vs {comparisonData.friend.avgExercises}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {comparisonData.user.monthlyStats.map(([month, userCount]) => {
                  const friendCount =
                    comparisonData.friend.monthlyStats.find(
                      ([m]) => m === month
                    )?.[1] || 0;
                      ...
                      <div className="flex-1 flex items-center space-x-2">
                        <div className="flex-1 bg-gray-200 rounded-full h-4 relative">
                          <div
                            ...
                            style={{
                              width: `${maxCount > 0 ? (userCount / maxCount) * 100 : 0}%`,
                            }}
                          />
                        ...
                        <div className="flex-1 bg-gray-200 rounded-full h-4 relative">
                          <div
                            ...
                            style={{
                              width: `${maxCount > 0 ? (friendCount / maxCount) * 100 : 0}%`,
                            }}
                          />
                          <span className="absolute inset-0 flex items-center justify-center text-xs font-medium text-white">
                            {friendCount}
