...
const WorkoutTemplates = ({
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      await saveCurrentWorkoutAsTemplate(exercises, templateName.trim(), templateDescription.trim());
  ...
  const handleCreateTemplate = async () => {
    ...
    try {
      const template = {
        ...
        totalSets: newTemplateExercises.reduce((acc, ex) => acc + (ex.sets?.length || 0), 0)
      };
  ...
  const removeExerciseFromNewTemplate = (exerciseId) => {
    setNewTemplateExercises(newTemplateExercises.filter(ex => ex.id !== exerciseId));
  };

  const updateExerciseInNewTemplate = (exerciseId, field, value) => {
    setNewTemplateExercises(newTemplateExercises.map(ex => 
      ...
      ex.id === exerciseId 
        ? {
            ...
            sets: ex.sets.map((set, idx) => 
              idx === setIndex ? { ...set, [field]: Math.max(0, parseInt(value) || 0) } : set
            )
          }
        : ex
    ));
  ...
  const getMuscleIcon = (muscle) => {
    switch (muscle) {
      case 'pectoraux': return <Dumbbell className="h-4 w-4 text-blue-600" />;
      case 'dos': return <Target className="h-4 w-4 text-green-600" />;
      case 'jambes': return <Target className="h-4 w-4 text-purple-600" />;
      case 'abdos': return <Target className="h-4 w-4 text-orange-600" />;
      case 'biceps': return <Dumbbell className="h-4 w-4 text-red-600" />;
      case 'triceps': return <Dumbbell className="h-4 w-4 text-indigo-600" />;
      case 'épaules': return <Target className="h-4 w-4 text-yellow-600" />;
      case 'cardio': return <Heart className="h-4 w-4 text-red-500" />;
      default: return <Dumbbell className="h-4 w-4 text-gray-600" />;
    }
  };
  ...
  const getMuscleIconLarge = (muscle) => {
    switch (muscle) {
      ...
      case 'jambes': return <Target className="h-8 w-8 text-white" />;
      case 'abdos': return <Target className="h-8 w-8 text-white" />;
      case 'biceps': return <Dumbbell className="h-8 w-8 text-white" />;
      case 'triceps': return <Dumbbell className="h-8 w-8 text-white" />;
      case 'épaules': return <Target className="h-8 w-8 text-white" />;
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="pt-6 mb-6 pl-6">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent">Mes Templates</h2>
              ...
              <div className="space-y-2 mb-4">
                {template.exercises.slice(0, 3).map((exercise, index) => (
                  <div key={index} className="flex items-center space-x-2 text-sm">
                    ...
                    <span className="text-gray-700 flex-1 truncate">{exercise.name}</span>
      ...
      {showEditModal && editingTemplate && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-800">Modifier le template</h3>
              <button
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6" style={{ height: '90vh' }}>
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h4 className="text-lg font-semibold text-gray-800">Exercices</h4>
                  <button
                ...
                {newTemplateExercises.length === 0 ? (
                  <div className="text-center py-8 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-400">Cliquez sur "Ajouter un exercice" pour commencer</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                      ...
                      <Card key={exercise.id} className="p-4">
                        <div className="flex justify-between items-start mb-3">
                          <div className="flex-1">
                            <input
                              type="text"
                              value={exercise.name}
                              onChange={(e) => updateExerciseInNewTemplate(exercise.id, 'name', e.target.value)}
                            ...
                            <div className="flex items-center space-x-2 mt-1">
                              ...
                              <span className="text-sm text-gray-600 capitalize font-medium">
                                {exercise.type === 'custom' ? 'Exercice personnalisé' : exercise.type}
                              </span>
                            </div>
                          </div>
                          <button
                        ...
                        <div className="space-y-2">
                          ...
                          {exercise.sets.map((set, setIndex) => (
                            <div key={setIndex} className="flex items-center gap-3 bg-gray-50 p-3 rounded-lg">
                              <span className="text-sm font-medium text-gray-600 w-8">#{setIndex + 1}</span>
                              <div className="flex gap-2 flex-1">
                                <div className="flex-1">
                                  ...
                                  <input
                                    type="number"
                                    value={set.reps}
                                    onChange={(e) => updateSetInExercise(exercise.id, setIndex, 'reps', e.target.value)}
                                ...
                                <div className="flex-1">
                                  ...
                                  <input
                                    type="number"
                                    value={set.weight}
                                    onChange={(e) => updateSetInExercise(exercise.id, setIndex, 'weight', e.target.value)}
                                    className="w-full border border-gray-200 rounded px-2 py-1 text-sm"
                                    min="0"
                                    step="0.5"
                                  />
                                ...
                                <div className="flex-1">
                                  ...
                                  <input
                                    type="number"
                                    value={set.duration}
                                    onChange={(e) => updateSetInExercise(exercise.id, setIndex, 'duration', e.target.value)}
                                    className="w-full border border-gray-200 rounded px-2 py-1 text-sm"
                                    min="0"
                                  />
                                </div>
                              ...
                              {exercise.sets.length > 1 && (
                                <button
                                  onClick={() => removeSetFromExercise(exercise.id, setIndex)}
            ...
            <div className="overflow-y-auto flex-1 pr-2" style={{ maxHeight: 'calc(90vh - 200px)' }}>
              ...
              <div className="space-y-6">
                {Object.entries(exerciseDatabase).map(([muscle, exerciseList]) => {
                  ...
                  if (filteredForMuscle.length === 0 && searchTerm) return null;

                  // Séparer favoris et non-favoris pour ce groupe
                  const favoritesInMuscle = filteredForMuscle.filter(exercise => favorites.includes(exercise));
                  const nonFavoritesInMuscle = filteredForMuscle.filter(exercise => !favorites.includes(exercise));

                  return (
                    <div key={muscle} className="bg-gray-50 rounded-2xl p-4">
                      {/* En-tête du groupe musculaire */}
                      <div className="flex items-center space-x-3 mb-4">
                        <div className={`p-3 rounded-xl ${muscle === 'cardio' ? 'bg-red-500' : 'bg-indigo-500'} shadow-lg`}>
                          {getMuscleIconLarge(muscle)}
                        </div>
                        <div>
                          <h4 className="font-bold text-gray-800 capitalize text-xl">{muscle}</h4>
                          <p className="text-sm text-gray-600">{filteredForMuscle.length} exercice{filteredForMuscle.length > 1 ? 's' : ''}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        ...
                        {favoritesInMuscle.map((exercise) => (
                          <div key={exercise} className="relative">
                            <button
                              onClick={() => addExerciseToNewTemplate(exercise, muscle)}
                            ...
                            >
                              <div className="flex items-center space-x-3">
                                <div className={`p-2 rounded-lg ${muscle === 'cardio' ? 'bg-red-100 text-red-600' : 'bg-indigo-100 text-indigo-600'}`}>
                                  {muscle === 'cardio' ? <Heart className="h-4 w-4" /> : <Dumbbell className="h-4 w-4" />}
                                </div>
                                <span className="flex-1 text-base font-medium">{exercise}</span>
              ...
              <div className="flex flex-col items-center justify-center gap-3 mt-6 w-full">
                <div className="w-full max-w-md bg-white rounded-2xl p-4 border-2 border-dashed border-indigo-300">
                  <h5 className="text-center font-semibold text-gray-700 mb-3">Exercice personnalisé</h5>
                  <input
                    type="text"
                  ...
                  <button
                    onClick={() => {
                      if (customExerciseName.trim()) {
                        addExerciseToNewTemplate(customExerciseName.trim(), 'custom');
                      }
                    }}
